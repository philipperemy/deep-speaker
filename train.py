...
def fit_model(dsm: DeepSpeakerModel, working_dir: str, max_length: int = NUM_FRAMES, batch_size=BATCH_SIZE):
    ...
    bx_test = []
    by_test = []
    for _ in tqdm(range(100), desc='Build test set'):
    ...
    dsm.m.fit(x=train_generator(), y=None, steps_per_epoch=500, shuffle=False,
              validation_data=test_generator(), validation_steps=len(bx_test),
